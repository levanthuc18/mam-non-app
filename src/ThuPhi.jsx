        ...
        if (fastMode) {
          ...
          return (
            <div key={r.hs.id} style={{ background: C.card, borderRadius: 12, border: `1px solid ${C.line}`, marginBottom: 8, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                                         <div style={{ flex: 1, minWidth: 0 }}>
                <div onClick={(e) => { e.stopPropagation(); onSelectStudent && onSelectStudent(r.hs.id); }} style={{ fontWeight: 700, fontSize: 15, color: C.ink, cursor: "pointer" }}>{r.hs.ten}{r.ps.suaCount > 0 && <span title="có khoản đã sửa" style={{ color: C.amber, fontSize: 12 }}> ⚠</span>}</div> 
              </div>
