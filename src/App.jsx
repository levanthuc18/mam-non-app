        ...
        <>
          ...
          {dow === 0 || isLeNgay ? (
            ...
            studentRows.map((r) => {
              ...
              return (
                <div key={r.hs.id} onClick={() => !disabled && toggle(r.hs.id, viewDay)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", marginBottom: 8, borderRadius: 14, background: C.card, border: `1.5px solid ${disabled ? C.gray : nghi ? C.coral : C.line}`, cursor: disabled ? "default" : "pointer", opacity: disabled ? 0.65 : 1 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: disabled ? C.graySoft : nghi ? C.coralSoft : C.greenSoft, color: disabled ? C.gray : nghi ? C.coral : C.green, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, flexShrink: 0 }}>{disabled ? "·" : (nghi ? "✕" : "✓")}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
