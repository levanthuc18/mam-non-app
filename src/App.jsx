...
function DiemDanhTab({ allRows, chipsLop, lopFilter, setLopFilter, search, setSearch, ddData, upDDData, leData, upLeData, year, month, locked, ddLockReason, isWide, ym, isGV, gvLopId, gvTen, students }) {
  ...
  useEffect(() => { setSaveState(null); setChiVang(false); }, [viewDay, mode]); // doi ngay/che do xem -> xoa trang thai cu
  ...
  return (
    <tr style={{ background: index % 2 ? "#FAFCFA" : "#fff" }}>
      ...
      {Array.from({ length: days }, (_, i) => i + 1).map((d) => {
        ...
        const chuaNhap = nhap !== 1 && d < nhap; // trước ngày nhập học
        const chuaNhapThang = nhap === 99;      // chưa nhập học trong tháng này
...
function HSDetail({ s, meta, ym, setHS, chuyenLop, inp, onRename }) {
...
function CaiDat({ meta, upMeta, students, upStudents, ym, reseedAll, isWide }) {
  ...
  const [showLeft, setShowLeft] = useState(false); // hien HS da Nghi hoc/Ra truong (mac dinh an)
  const [thaoTacOpen, setThaoTacOpen] = useState(false);
  const [ttView, setTtView] = useState("menu"); // menu|khac|lop|tt|thu|pl|ngay|ratruong|xoa
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
              <div style={{ fontSize: 11.5, color: C.sub }}>
                {filteredHS.length} HS đang hiển thị
              </div>
