...
export default function App() {
  ...
  useEffect(() => {
    ...
    const snh = soNgayHoc(year, month, leData);
    let changed = false;
    const fees = { ...mData.fees };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: font.body, color: C.ink }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "14px 14px 92px" }}>
        ...
        {tab === "dash" && mData && (
          <DashTab {...{ tk, mData, upMData, month, year, locked, meta, allRows, delThang, students, ym, upMeta }} />
        )}
        {tab === "no" && (
...
function DiemDanhTab({ allRows, chipsLop, lopFilter, setLopFilter, search, setSearch, ddData, upDDData, leData, upLeData, year, month, locked, ddLockReason, isWide, ym, isGV, gvLopId, gvTen, students }) {
  ...
  const studentRows = useMemo(() => {
    const s = noDau(search);
    const effLop = isGV ? gvLopId : lopFilter; // [PQ] GV chi thay lop minh
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        <button onClick={() => setMode("ngay")} style={{ flex: 1, padding: "8px 0", borderRadius: 10, border: `1.5px solid ${mode === "ngay" ? C.pine : C.line}`, background: mode === "ngay" ? C.pine : C.card, color: mode === "ngay" ? "#fff" : C.sub, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: font.body }}>Theo ngày</button>
        <button onClick={() => setMode("thang")} style={{ flex: 1, padding: "8px 0", borderRadius: 10, border: `1.5px solid ${mode === "thang" ? C.pine : C.line}`, background: mode === "thang" ? C.pine : C.card, color: mode === "thang" ? "#fff" : C.sub, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: font.body }}>Cả tháng (bảng)</button>
      </div>
        ...
        <>
          <Card style={{ marginBottom: 12, padding: "10px 12px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <button onClick={() => setViewDay(Math.max(1, viewDay - 1))} style={{ fontSize: 20, color: C.pine, border: "none", background: "none", cursor: "pointer", padding: "0 6px" }}>‹</button>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 16, color: dow === 0 ? C.gray : isLeNgay ? C.amber : C.ink }}>{dowLabel}, {viewDay}/{month}</div>
                <div style={{ fontSize: 11.5, color: C.sub }}>{dow === 0 ? "Chủ nhật — nghỉ" : isLeNgay ? "Ngày lễ — nghỉ cả trường" : `Nghỉ: ${soNghiNgay}/${studentRows.length} cháu`}{isCurMonth && viewDay === today.getDate() ? " · hôm nay" : ""}</div>
              </div>
              <button onClick={() => setViewDay(Math.min(days, viewDay + 1))} style={{ fontSize: 20, color: C.pine, border: "none", background: "none", cursor: "pointer", padding: "0 6px" }}>›</button>
            </div>
            <input type="range" min={1} max={days} value={viewDay} onChange={(e) => setViewDay(Number(e.target.value))} style={{ width: "100%", marginTop: 8, accentColor: C.pine }} />
          ...
          {dow === 0 || isLeNgay ? (
            ...
            studentRows.map((r) => {
              const nghi = att[r.hs.id]?.[viewDay];
              return (
                <div key={r.hs.id} onClick={() => toggle(r.hs.id, viewDay)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", marginBottom: 8, borderRadius: 14, background: C.card, border: `1.5px solid ${nghi ? C.coral : C.line}`, cursor: locked ? "default" : "pointer" }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: nghi ? C.coralSoft : C.greenSoft, color: nghi ? C.coral : C.green, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, flexShrink: 0 }}>{nghi ? "✕" : "✓"}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 15 }}>{r.hs.ten}</div>
                    <div style={{ fontSize: 12, color: C.sub }}>{r.lop?.ten}</div>
                  </div>
                  <Badge s={nghi ? { t: "Nghỉ", c: C.coral, bg: C.coralSoft } : { t: "Đi học", c: C.green, bg: C.greenSoft }} />
                </div>
              );
            })
          )}
...
function DashTab({ tk, mData, upMData, month, year, locked, meta, allRows, delThang, students, ym, upMeta }) {
  ...
  const themCoDinhMau = () => {
    ...
    if (!co.length) { toast("Đã có đủ 5 khoản cố định."); return; }
    ...
    <>
      ...
      <Card style={{ marginBottom: 12, padding: 0 }}>
        ...
        <div style={{ display: "flex", gap: 14, fontSize: 12.5, color: C.sub, marginBottom: 10, flexWrap: "wrap" }}>
          ...
          {!locked && <button onClick={themCoDinhMau} style={{ marginLeft: "auto", background: C.pineSoft, color: C.pine, border: "none", borderRadius: 8, padding: "5px 10px", fontSize: 11.5, fontWeight: 700, cursor: "pointer" }}>+ 5 khoản cố định</button>}
        </div>
        {cp.map((c) => {
          ...
          return (
            <div key={c.id} style={{ padding: "9px 0", borderTop: `1px solid ${C.line}` }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", fontSize: 12.5, color: C.sub }}>
                <span>Phải trả <b style={{ color: C.ink }}>{fmt(e)}</b> · đã trả</span>
...
// [IMPORT] Nhap hang loat hoc sinh tu CSV/Excel hoac dan noi dung
      ...
      {sec === "hs" && (
        <>
          ...
          <div style={{
            ...
            {/* Search + Chọn nhiều cùng hàng */}
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <SearchBar value={hsSearch} onChange={setHsSearch} />
              </div>
              <button onClick={() => { setBulkMode((v) => !v); setSelectedHS([]); }} style={{ padding: "7px 12px", borderRadius: 8, border: `1.5px solid ${bulkMode ? C.pine : C.line}`, background: bulkMode ? C.pine : C.card, color: bulkMode ? "#fff" : C.sub, fontWeight: 700, fontSize: 12.5, cursor: "pointer", fontFamily: font.body, flexShrink: 0, height: 40 }}>{bulkMode ? "⛔ Thoát" : "☑ Chọn nhiều"}</button>
            </div>

            {/* Chips lớp ra ngoài như cũ */}
          ...
          {/* ===== BOTTOM SHEET: FILTER (chỉ còn trạng thái) ===== */}
          {showFilterSheet && (
            <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              ...
              <div style={{ background: C.card, borderRadius: "20px 20px 0 0", padding: "16px 16px 24px", maxHeight: "70vh", overflowY: "auto", boxShadow: "0 -4px 20px rgba(0,0,0,.15)" }}>
                ...
                <div style={{ fontSize: 12, color: C.sub, marginBottom: 8, fontWeight: 600 }}>Trạng thái</div>
            ...
            return (
              <Card key={s.id} style={{ marginBottom: 8, padding: 0, overflow: "hidden", border: bulkMode && isSel ? `2px solid ${C.pine}` : undefined }}>
                <div onClick={() => { if (bulkMode) setSelectedHS((prev) => isSel ? prev.filter((id) => id !== s.id) : [...prev, s.id]); else setEditHS(edit ? null : s.id); }} style={{ padding: "12px 14px", cursor: "pointer" }}>
                  {/* Hàng 1: Tên + A/B + Xóa */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: C.ink, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.ten}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                      {!bulkMode && <ABBtn val={s.nguoiThu} set={(p) => setHS(s.id, { nguoiThu: p })} small />}
                      {bulkMode && <div style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${isSel ? C.pine : C.line}`, background: isSel ? C.pine : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 800 }}>{isSel ? "✓" : ""}</div>}
                  ...
                  {/* Hàng 2: Lớp · PL · Trạng thái */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 12, color: C.sub }}>{meta.classes.find((c) => c.id === lh)?.ten} · {s.pl}</span>
