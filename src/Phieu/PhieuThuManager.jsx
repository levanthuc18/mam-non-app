...
export function PhieuThuManager({ allRows, meta, month, year, mData, upMData, upMeta }) {
  ...
  if (mode === "single" && singleRow) {
    return (
      <div>
        <button onClick={() => setMode("manager")} className="no-print" style={{ marginBottom: 12, padding: "8px 14px", borderRadius: 10, border: `1.5px solid ${C.line}`, background: C.card, color: C.ink, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          ❮ Quay lại quản lý in
        </button>
        <PhieuThu
          phieuRow={singleRow}
  ...
  const sidebar = (
    <div className="sidebar" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <button
          onClick={() => { const first = allRows.find((r) => r.coRec); if (first) { setSingleId(first.hs.id); setMode("single"); } }}
          style={{ padding: "12px 8px", borderRadius: 12, border: "none", background: C.pine, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}
        >
          <span style={{ fontSize: 16 }}>🖨</span>
          <span>In phiếu hiện tại</span>
          <span style={{ fontSize: 10, opacity: 0.8, fontWeight: 500 }}>{singleRow?.rec?.bienLai || "Chưa có BL"}</span>
        </button>
        <button
          onClick={() => { setPreviewPage(0); }}
          style={{ padding: "12px 8px", borderRadius: 12, border: `2px solid ${C.amber}`, background: "#fff", color: C.amber, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}
        >
          <span style={{ fontSize: 16 }}>📦</span>
          <span>In hàng loạt</span>
          <span style={{ fontSize: 10, fontWeight: 500 }}>{rowsToPrint.length} phiếu · {meta.classes.length} lớp</span>
        </button>
      </div>

      <div style={{ background: C.pineSoft, border: `1.5px solid #BFE0D4`, borderRadius: 12, padding: "10px 12px" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 2 }}>
          <span style={{ fontSize: 13, color: C.pine }}>📋</span>
          <span style={{ fontFamily: font.display, fontWeight: 700, fontSize: 15, color: C.pine }}><AnimatedCounter value={rowsToPrint.length} /></span>
          <span style={{ fontSize: 12, color: C.pine }}>phiếu sẽ in</span>
        </div>
        <div style={{ fontSize: 12, color: C.sub, fontWeight: 600 }}>
          💰 Tổng cần thu: <span style={{ color: C.coral }}>{fmt(tongNo)} đ</span>
        </div>
        <div style={{ fontSize: 11, color: C.sub, marginTop: 2 }}>
          Tổng phải thu: {fmt(tongPhaiThu)} đ
        </div>
      </div>

      <div style={{ background: C.card, border: `1.5px solid ${C.line}`, borderRadius: 12, padding: "12px 14px" }}>
        <div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 13, color: C.ink, marginBottom: 10 }}>BỘ LỌC TIẾT KIỆM GIẤY</div>
        {[
          ...
          <label key={f.key} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer", fontSize: 13, color: C.ink }}>
            <input
              type="checkbox"
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer", fontSize: 13, color: C.ink }}>
          <input
            type="checkbox"
      ...
      <div>
        <div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 13, color: C.ink, marginBottom: 8 }}>CHỌN LỚP</div>
        <select
          value={selectedLop}
      ...
      <div style={{ position: "relative" }}>
        <button
          ...
          style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1.5px solid ${C.line}`, background: C.card, color: C.ink, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}
        >
          <span>📤 Xuất dữ liệu</span>
          <span style={{ fontSize: 10 }}>▾</span>
        </button>
        {showExportMenu && (
          <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#fff", border: `1.5px solid ${C.line}`, borderRadius: 10, boxShadow: "0 4px 16px rgba(0,0,0,.1)", zIndex: 50, overflow: "hidden" }}>
            {[
              ...
              <button key={i} style={{ width: "100%", textAlign: "left", padding: "10px 12px", border: "none", background: "none", cursor: "pointer", fontSize: 13, color: C.ink, display: "flex", alignItems: "center", gap: 8, borderBottom: i < 3 ? `1px solid ${C.line}` : "none" }} onClick={() => setShowExportMenu(false)}>
      ...
      >
        <span>🖨 IN {rowsToPrint.length} PHIẾU</span>
        <span style={{ fontSize: 12, opacity: 0.85, fontWeight: 500 }}>{fmt(tongNo)} đ · ({totalPages} trang)</span>
      </button>
    </div>
  );
    ...
    <>
      <style>{`
        @media print {
          ...
          .phieu-page { page-break-after: always; break-after: page; }
          .phieu-page:last-child { page-break-after: auto; break-after: auto; }
          .no-print { display: none !important; }
        }
        @media (min-width: 820px) {
          .phieu-manager { display: grid; grid-template-columns: 340px 1fr; max-width: 1200px; margin: 0 auto; gap: 20px; }
          .sidebar { position: sticky; top: 14px; height: calc(100vh - 28px); overflow-y: auto; padding-right: 4px; }
          .preview-area { overflow-y: auto; padding-left: 4px; }
        }
        @media (max-width: 819px) {
          .phieu-manager { display: flex; flex-direction: column; }
        }
      `}</style>
      ...
      <div className="print-area no-print" style={{ display: "none" }}>
        {rowsToPrint.map((r) => (
      ...
      {showIssues && (
        <div onClick={() => setShowIssues(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 150, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420, width: "100%", background: "#fff", borderRadius: 16, padding: 20, maxHeight: "80vh", overflow: "auto" }}>
            <div style={{ fontFamily: font.display, fontWeight: 700, fontSize: 15, color: C.ink, marginBottom: 12 }}>⚠️ Chi tiết lỗi cần sửa</div>
            <PreflightCheck rows={rowsToPrint} meta={meta} />
            <button onClick={() => setShowIssues(false)} style={{ width: "100%", marginTop: 14, padding: "12px 0", borderRadius: 11, border: "none", background: C.pine, color: "#fff", fontFamily: font.display, fontWeight: 700, fontSize: 15, cursor: "pointer" }}>✓ Đã hiểu</button>
          </div>
        </div>
      )}
    </>
