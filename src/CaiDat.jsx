  ...
  return (
    <>
      <div style={{ display: "flex", gap: 6, marginBottom: 10, overflowX: "auto", paddingBottom: 4 }}>
        {[
          ["hs", "Học sinh"], ["lop", "Lớp"], ["gv", "Giáo viên"], ["bank", "Tài khoản"], ["dk", "Số dư đầu kỳ"], ["backup", "Sao lưu"], ["log", "Nhật ký"], ["data", "Dữ liệu"],
        ].map(([k, l]) => (
      ...
      {sec === "hs" && (
        <>
          ...
          <div style={{ position: "sticky", top: 0, zIndex: 25, background: headerShrunk ? "rgba(245,247,243,.95)" : "transparent", backdropFilter: headerShrunk ? "blur(6px)" : "none", margin: "0 -14px", padding: headerShrunk ? "8px 14px" : "0 0 10px", borderBottom: headerShrunk ? `1px solid ${C.line}` : "none", transition: "all .2s ease" }}>
            <div style={{ display: "flex", gap: 8, marginBottom: headerShrunk ? 8 : 12 }}>
              <button onClick={() => setShowAddHS((v) => !v)} style={{ flex: 1, padding: headerShrunk ? "9px 6px" : "11px 8px", borderRadius: 12, border: `1.5px solid ${showAddHS ? C.pine : C.line}`, background: showAddHS ? C.pine : C.card, color: showAddHS ? "#fff" : C.pine, fontFamily: font.display, fontWeight: 700, fontSize: headerShrunk ? 12 : 13.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>{headerShrunk ? "＋" : (showAddHS ? "▲ Thu gọn" : "＋ Thêm 1 HS")}</button>
              <button onClick={() => setShowImport((v) => !v)} style={{ flex: 1, padding: headerShrunk ? "9px 6px" : "11px 8px", borderRadius: 12, border: `1.5px solid ${showImport ? C.blueA : C.line}`, background: showImport ? C.blueA : C.card, color: showImport ? "#fff" : C.blueA, fontFamily: font.display, fontWeight: 700, fontSize: headerShrunk ? 12 : 13.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>{headerShrunk ? "📥" : (showImport ? "▲ Thu gọn" : "📥 Nhập hàng loạt")}</button>
            </div>
            ...
            {bulkMode && (
              <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
                <button onClick={toggleSelectAll} style={{ padding: "7px 12px", borderRadius: 8, border: `1.5px solid ${allFilteredSelected ? C.pine : C.line}`, background: allFilteredSelected ? C.pineSoft : C.card, color: C.pine, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: font.body }}>{allFilteredSelected ? "✕ Bỏ chọn" : "✓ Chọn tất cả"}</button>
                {selectedHS.length > 0 && <span style={{ fontSize: 12, color: C.sub }}><b>{selectedHS.length}</b> đã chọn</span>}
                {selectedHS.length > 0 && <button onClick={() => { setTtView("menu"); setThaoTacOpen(true); }} style={{ marginLeft: "auto", padding: "7px 14px", borderRadius: 8, border: "none", background: C.pine, color: "#fff", fontWeight: 700, fontSize: 12.5, cursor: "pointer", fontFamily: font.body }}>⚙️ Thao tác</button>}
              </div>
            )}
            ...
            <BottomSheet open={thaoTacOpen} onClose={closeThaoTac} title={ttView === "menu" ? `Thao tác cho ${selectedHS.length} HS` : ttView === "khac" ? `Khác — ${selectedHS.length} HS` : ttView === "lop" ? `Chuyển ${selectedHS.length} HS sang lớp` : ttView === "tt" ? `Đổi trạng thái ${selectedHS.length} HS` : ttView === "thu" ? `Đổi người thu ${selectedHS.length} HS` : ttView === "pl" ? `Đổi phân loại ${selectedHS.length} HS` : ttView === "ngay" ? `Đặt ngày nhập học ${selectedHS.length} HS` : ttView === "ratruong" ? `Cho ${selectedHS.length} HS ra trường` : `Xóa vĩnh viễn ${selectedHS.length} HS`}>
              ...
              {ttView === "ngay" && (<div>
                ...
                <button onClick={() => bulkPatch({ ngayNhapHoc: bulkNgayNhap }, `Đặt ngày nhập học hàng loạt ${selectedHS.length} HS → ${bulkNgayNhap}`, `Đã đặt ngày nhập học cho ${selectedHS.length} HS`)} style={{ width: "100%", padding: "12px 0", borderRadius: 11, border: "none", background: C.pine, color: "#fff", fontWeight: 700, fontSize: 14.5, cursor: "pointer", fontFamily: font.body }}>Áp dụng</button>
              </div>)}
              {ttView === "ratruong" && (<div>
                <button onClick={() => setTtView("khac")} style={{ border: "none", background: "none", color: C.pine, fontWeight: 700, fontSize: 13, cursor: "pointer", marginBottom: 10, padding: 0 }}>‹ Quay lại</button>
                <div style={{ fontSize: 12.5, color: C.sub, marginBottom: 8, lineHeight: 1.5 }}>Sẽ đặt ngày nghỉ + chuyển trạng thái sang <b>Ra trường</b> cho {selectedHS.length} HS. Các em này sẽ không bị tính phí từ tháng sau và bị ẩn khỏi danh sách (vẫn giữ lịch sử).</div>
              ...
              {ttView === "xoa" && (<div>
                ...
                <div style={{ fontSize: 12.5, color: C.sub, marginBottom: 10, lineHeight: 1.5 }}>Mất hẳn dữ liệu thu phí + điểm danh của các em này, <b>không xem lại được</b>, kể cả tháng đã chốt (số liệu tháng đã chốt có thể sai lệch). Nên dùng <b>"Cho ra trường"</b> để giữ lịch sử. Gõ <b>XOA</b> để xác nhận xóa thật.</div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
              <div style={{ fontSize: 11.5, color: C.sub }}>{filteredHS.length} HS đang hiển thị</div>
              <button onClick={() => setShowLeft((v) => !v)} style={{ flexShrink: 0, padding: "4px 10px", borderRadius: 8, border: `1.5px solid ${showLeft ? C.pine : C.line}`, background: showLeft ? C.pineSoft : C.card, color: showLeft ? C.pine : C.sub, fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: font.body }}>{showLeft ? "✓ Đang hiện HS đã nghỉ" : "Hiện HS đã nghỉ/ra trường"}</button>
            </div>
          </div>

          {(() => {
            const filtered = filteredHS;
              ...
              {shown.map((s, idx) => {
                ...
                return (
                  <Card key={s.id} style={{ marginBottom: 8, padding: 0, overflow: "hidden", border: bulkMode && isSel ? `2px solid ${C.pine}` : isDragging ? `2px dashed ${C.pine}` : undefined, opacity: isDragging ? 0.6 : 1, transition: "margin .2s ease" }}>
                    {dragOverId === s.id && dragOverPos === "before" && (<div style={{ height: 4, background: C.pine, borderRadius: 2, margin: "2px 8px", boxShadow: "0 1px 4px rgba(23,107,91,.35)" }} />)}
                    <div
                    ...
                    {dragOverId === s.id && dragOverPos === "after" && (<div style={{ height: 4, background: C.pine, borderRadius: 2, margin: "2px 8px", boxShadow: "0 1px 4px rgba(23,107,91,.35)" }} />)}
                    {edit && !bulkMode && !reorderMode && (
                      <div style={{ borderTop: `1px dashed ${C.line}`, padding: "12px", background: "#FBFDFB" }}>
                        <HSDetail key={s.id} s={s} meta={meta} ym={ym} setHS={setHS} chuyenLop={chuyenLop} inp={inp} onRename={renameHS} />
                      </div>
                    )}
                  </Card>
                );
