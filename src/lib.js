...
import { C, font, fmt, noDau, PL_COLOR } from "./lib.js";
...
export function NumInput({ value, onChange, w = 70, disabled, warn }) {
  export const [focused, setFocused] = useState(false);
  export const display = focused
...
export function useStickyShrink() {
  export const sentinelRef = useRef(null);
  export const [shrunk, setShrunk] = useState(false);
  useEffect(() => {
    export const el = sentinelRef.current;
    if (!el) return;
    export const ob = new IntersectionObserver(([e]) => setShrunk(!e.isIntersecting), { root: null, threshold: 0 });
    ob.observe(el);
